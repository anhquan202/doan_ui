...
const props = defineProps<{ modelValue: { room_type?: string; price?: number; description?: string } }>()
    ...
    <div class="grid grid-cols-2 gap-4">
        ...
        <div class="relative">
          <button type="button" @click="toggleDropdown"
            ...
            <span :class="props.modelValue?.room_type ? 'text-slate-800' : 'text-slate-400'">
              {{options.find(o => o.value === props.modelValue?.room_type)?.label || '-- Chọn loại phòng --'}}
            </span>
          ...
          <div v-if="isOpen" class="absolute z-10 w-full mt-2 bg-white rounded-xl shadow-xl border border-slate-200">
            <button v-for="option in options" :key="option.value" type="button"
              @click="() => { updateField('room_type', option.value); isOpen = false }"
      ...
      <div>
        ...
        <input type="number" min="0" step="1000" :value="props.modelValue?.price"
          @input="updateField('price', +$event.target.value)" placeholder="VD: 500000"
      ...
      <div class="col-span-2">
        ...
        <textarea rows="4" :value="props.modelValue?.description"
          @input="updateField('description', $event.target.value)" placeholder="Mô tả ngắn gọn về phòng..."
